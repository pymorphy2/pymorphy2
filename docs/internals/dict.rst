...
соответствующая им грамматическая информация ("тег").
Первая форма считается "нормальной". В словаре около 400тыс. лемм
и 5млн отдельных слов.

.. note::

    С сайта OpenCorpora_ для скачивания доступны plaintext- и XML-версии
    словаря. В pymorphy2 используется XML-версия, но (для понятности) тут
...
Каждым тегом (например, ``NOUN,anim,masc sing,nomn``) обычно помечено
более одного слова (часто - очень много слов). Хранить строку целиком
для всех 5млн слов накладно по 2 причинам:
...
- строки нельзя хранить в `array.array`_, а у list накладные расходы выше,
  т.к. он в питоне реализован как массив указателей на объекты (об этом дальше
  еще будет).
...
У слов в лемме есть неизменяемое начало ("стем") и изменяемое
...
определяется как "окончание", "номер грамматической информации" и "префикс".
...
бы их не хранить по многу раз (а еще лучше - создавать поменьше питоньих
объектов для них), поэтому все возможные окончания хранятся в отдельном массиве,
а в парадигме указывается только номер окончания; с префиксами то же самое.
...
(индексация с 0) будет лежать в элементе массива с номером ``12/3 + 2 = 6``.
...
``tag<N>`` - тег (грамматическая информация, набор граммем):
...
с возможными окончаниями, грамматической информацией (тегами)
и префисками соответственно.
...
со всеми сопутствующими данными занимает около 20Мб оперативной памяти;
скорость разбора - порядка 50..150 тыс слов/сек. Для сравнения:
...
* в :ref:`первом варианте <2trie>` формата словарей pymorphy2
  ...
  при 30M памяти или 2-5 тыс слов/сек при 5Мб памяти (без предсказателя).
